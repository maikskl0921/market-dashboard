...
with c_sel1:
    ...
    if 'selected_country' not in st.session_state:
        st.session_state.selected_country = "요약"
        
    country_options = ["요약", "미국", "한국"]
...
def calculate_latest_signals(df, df_kr):
    ...
    def get_cnt_color(cnt):
        ...
        c = colors.get(min(max(1, int(cnt)), 7), '#800080')
    ...
    if fgi_us <= 9 and vix_us >= 26:
        bg, fg = to_rgba5_and_fg('#595959')
        us_low_panic = {"detected": True, "items": [("패닉", fg)], "bg_color": bg}
    elif 10 <= fgi_us <= 19 and 22 <= vix_us <= 25:
        bg, fg = to_rgba5_and_fg('#E06666')
        us_low_panic = {"detected": True, "items": [("패닉", fg)], "bg_color": bg}
    elif 20 <= fgi_us <= 29 and 18 <= vix_us <= 21:
        bg, fg = to_rgba5_and_fg('#FFD700')
        us_low_panic = {"detected": True, "items": [("패닉", fg)], "bg_color": bg}
    elif 30 <= fgi_us <= 39 and 14 <= vix_us <= 17:
        bg, fg = to_rgba5_and_fg('#A9D08E')
        us_low_panic = {"detected": True, "items": [("패닉", fg)], "bg_color": bg}
    ...
    if cnt_us_low_multi >= 1:
        c = '#E06666' if cnt_us_low_multi <= 7 else '#FF8C00' if cnt_us_low_multi <= 14 else '#FFD700' if cnt_us_low_multi <= 21 else '#A9D08E' if cnt_us_low_multi <= 28 else '#87CEEB' if cnt_us_low_multi <= 35 else '#000080' if cnt_us_low_multi <= 42 else '#800080'
    ...
    if cnt_us_high_multi >= 1:
        c = '#E06666' if cnt_us_high_multi <= 7 else '#FF8C00' if cnt_us_high_multi <= 14 else '#FFD700' if cnt_us_high_multi <= 21 else '#A9D08E' if cnt_us_high_multi <= 28 else '#87CEEB' if cnt_us_high_multi <= 35 else '#000080' if cnt_us_high_multi <= 42 else '#800080'
        bg, fg = to_rgba5_and_fg(c)
    ...
    if ((fgi_kr <= 18) and (vk_kr >= 26)) or ((fgi_kr == 50) and (vk_kr >= 30) and (pct_b_kr <= 0.05)):
        bg, fg = to_rgba5_and_fg('#595959')
        kr_low_panic = {"detected": True, "items": [("패닉", fg)], "bg_color": bg}
    ...
    if cnt_kr_low_multi >= 1:
        c = '#E06666' if cnt_kr_low_multi <= 7 else '#FF8C00' if cnt_kr_low_multi <= 14 else '#FFD700' if cnt_kr_low_multi <= 21 else '#A9D08E' if cnt_kr_low_multi <= 28 else '#87CEEB' if cnt_kr_low_multi <= 35 else '#000080' if cnt_kr_low_multi <= 42 else '#800080'
        bg, fg = to_rgba5_and_fg(c)
    ...
    if cnt_kr_high_multi >= 1:
        c = '#E06666' if cnt_kr_high_multi <= 7 else '#FF8C00' if cnt_kr_high_multi <= 14 else '#FFD700' if cnt_kr_high_multi <= 21 else '#A9D08E' if cnt_kr_high_multi <= 28 else '#87CEEB' if cnt_kr_high_multi <= 35 else '#000080' if cnt_kr_high_multi <= 42 else '#800080'
        bg, fg = to_rgba5_and_fg(c)
...
def update_detection_history(df, df_kr):
    ...
    kr_date_str = str(df_kr.index[-1])[:10]
    last_date_str = min(us_date_str, kr_date_str)
...
if selected_country == "요약":
    summary_tabs = st.tabs(['감지이력', '미국저점', '미국고점', '한국저점', '한국고점'])
    with summary_tabs[0]:
    ...
    with summary_tabs[1]:
        ...
        base_img_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "images", "summary_us_low")
        os.makedirs(base_img_dir, exist_ok=True)

        # Embedded Base64 dictionary for 100% deployment guarantee
        EMBEDDED_IMAGES = {'1_panic.png': '', '2_slope.png': '', '3_slope_angle.png': '', '4_multi.png': '', '5_unified.png': '', '6_gamma_single.png': '', '7_gamma_hybrid.png': ''}

        def get_img_b64(fname):
            filePath = os.path.join(base_img_dir, fname)
            if os.path.exists(filePath):
                try:
                    ...
                    pass
            return EMBEDDED_IMAGES.get(fname, "")

        # 1. 미국저점 공탐변동
        st.markdown("<h4 style='color:#1F4E79;margin-bottom:4px;'>1. 미국저점 공탐변동</h4>", unsafe_allow_html=True)
        b64_1 = get_img_b64("1_panic.png")
        if b64_1:
            st.markdown(f"<div style='overflow-x:auto;margin-bottom:1rem;'><img src='data:image/png;base64,{b64_1}' style='max-width:100%;height:auto;display:block;'></div>", unsafe_allow_html=True)

        # 2. 미국저점 슬로프합
        st.markdown("<h4 style='color:#1F4E79;margin-bottom:4px;'>2. 미국저점 슬로프합</h4>", unsafe_allow_html=True)
        b64_2 = get_img_b64("2_slope.png")
        if b64_2:
            st.markdown(f"<div style='overflow-x:auto;margin-bottom:1rem;'><img src='data:image/png;base64,{b64_2}' style='max-width:100%;height:auto;display:block;'></div>", unsafe_allow_html=True)

        # 3. 미국저점 기울기합
        st.markdown("<h4 style='color:#1F4E79;margin-bottom:4px;'>3. 미국저점 기울기합</h4>", unsafe_allow_html=True)
        b64_3 = get_img_b64("3_slope_angle.png")
        if b64_3:
            st.markdown(f"<div style='overflow-x:auto;margin-bottom:1rem;'><img src='data:image/png;base64,{b64_3}' style='max-width:100%;height:auto;display:block;'></div>", unsafe_allow_html=True)

        # 4. 미국저점 다중지표
        st.markdown("<h4 style='color:#1F4E79;margin-bottom:4px;'>4. 미국저점 다중지표</h4>", unsafe_allow_html=True)
        b64_4 = get_img_b64("4_multi.png")
        if b64_4:
            st.markdown(f"<div style='overflow-x:auto;margin-bottom:1rem;'><img src='data:image/png;base64,{b64_4}' style='max-width:100%;height:auto;display:block;'></div>", unsafe_allow_html=True)

        # 5. 미국저점 통합지표
        st.markdown("<h4 style='color:#1F4E79;margin-bottom:4px;'>5. 미국저점 통합지표</h4>", unsafe_allow_html=True)
        b64_5 = get_img_b64("5_unified.png")
        if b64_5:
            st.markdown(f"<div style='overflow-x:auto;margin-bottom:1rem;'><img src='data:image/png;base64,{b64_5}' style='max-width:100%;height:auto;display:block;'></div>", unsafe_allow_html=True)

        # 6. 미국저점 감마풋콜 (단독)
        st.markdown("<h4 style='color:#1F4E79;margin-bottom:4px;'>6. 미국저점 감마풋콜 (단독)</h4>", unsafe_allow_html=True)
        b64_6 = get_img_b64("6_gamma_single.png")
        if b64_6:
            st.markdown(f"<div style='overflow-x:auto;margin-bottom:1rem;'><img src='data:image/png;base64,{b64_6}' style='max-width:100%;height:auto;display:block;'></div>", unsafe_allow_html=True)

        # 7. 미국저점 감마풋콜 (혼합)
        st.markdown("<h4 style='color:#1F4E79;margin-bottom:4px;'>7. 미국저점 감마풋콜 (혼합)</h4>", unsafe_allow_html=True)
        b64_7 = get_img_b64("7_gamma_hybrid.png")
        if b64_7:
            st.markdown(f"<div style='overflow-x:auto;margin-bottom:1rem;'><img src='data:image/png;base64,{b64_7}' style='max-width:100%;height:auto;display:block;'></div>", unsafe_allow_html=True)

    with summary_tabs[2]:
        st.markdown("<h3 style='text-align:center;'>📊 미국 고점지표 요약</h3>", unsafe_allow_html=True)
        st.info("미국 고점지표 요약 신규 개발 준비 중...")

    with summary_tabs[3]:
        st.markdown("<h3 style='text-align:center;'>📊 한국 저점지표 요약</h3>", unsafe_allow_html=True)
        st.info("한국 저점지표 요약 신규 개발 준비 중...")

    with summary_tabs[4]:
        st.markdown("<h3 style='text-align:center;'>📊 한국 고점지표 요약</h3>", unsafe_allow_html=True)
        st.info("한국 고점지표 요약 신규 개발 준비 중...")
...
with tabs[0]:
    ...
    with bottom_sub_tabs[2]:
        if selected_country == "미국":
            ...
            selected_bottom_slopes_test = st.multiselect("📊 표시할 슬로프 차트 선택 (다중 선택 가능)", bottom_slope_options_test, default=["슬로프통합"], key="bottom_slope_test_multiselect")
            ...
            if not selected_bottom_slopes_test:
                ...
                chart_height_test = max(400, num_charts_test * 300)
            ...
            v2_slope_rainbow_verify_test = {
                ...
                "파란색 (5개 감지)": (df['slope_detect_count_test'] == 5, "동시 감지 5개"),
            ...
            selected_bottom_slopes_test_kr = st.multiselect("📊 표시할 슬로프 차트 선택 (다중 선택 가능)", bottom_slope_options_test_kr, default=["슬로프통합"], key="bottom_slope_multiselect_test_kr")
            ...
            if not selected_bottom_slopes_test_kr:
                ...
                chart_height_test_kr = max(400, num_charts_test_kr * 300)
            ...
            v2_slope_rainbow_verify_test_kr = {
                ...
                "파란색 (5개 감지)": (df_kr['slope_detect_count_test_kr'] == 5, "동시 감지 5개"),
    ...
    with sub_tabs[0]:
        ...
        if not df_mon.empty:
            ...
            if not df_mon.empty:
                ...
                bar_colors = ['rgba(255, 105, 180, 0.5)' if val >= 0 else 'rgba(135, 206, 235, 0.5)' for val in df_mon_plot['Foreign']]
...
with tabs[1]:
    if selected_country == "미국":
        ...
        with top_sub_tabs[2]:
            ...
            bottom_slope_options_test = ["슬로프통합", "10일합", "20일합", "30일합", "40일합", "50일합", "60일합", "70일합"]
            selected_slopes_test = st.multiselect("📊 표시할 슬로프 차트 선택 (다중 선택 가능)", bottom_slope_options_test, default=["슬로프통합"], key="top_slope_multiselect_test")
        ...
        with top_sub_tabs_kr[2]:
            ...
            selected_slopes_kr_top_test = st.multiselect("📊 표시할 슬로프 차트 선택 (다중 선택 가능)", bottom_slope_options_kr_top_test, default=["슬로프통합"], key="top_slope_multiselect_test_kr")
            ...
            if not selected_slopes_kr_top_test:
                ...
                chart_height_kr_test = max(400, num_charts_kr_top_test * 300)
            ...
            slope_multi_conditions_kr_top_test = {
                ...
                "**파란색 (5개 감지)**": (df_top_kr['slope_detect_count_top_kr_test'] >= 5, "동시 감지 5개"),
